...
def env_exec():
	...
	connect_and_commands(device_list)
...
def connect_and_commands(equipment):
	...
	try:
		with netmiko.ConnectHandler(**equipment) as connection:
			command_string = f'show ip interfade brief | i {regex}'
			...
			if re_result_1:
				...
				with open("Logs\\change_ip_helper_address.txt", 'a') as f:
				...
				with open("Logs\\change_ip_helper_address.txt", 'a') as f:
					f.write(f"{equipment['secret']} ({equipment['ip']}): Interface não encontrada")
		...
		main_logger.info(f"{equipment['secret']} ({equipment['ip']}): Alterado")
