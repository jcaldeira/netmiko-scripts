...
def main():
    ...
    if sys.platform in ("linux", "darwin"):
        ...
        cfg_file = 'Config files/SWs-PRESI_more-buffer.ios'
    elif sys.platform == "win32":
    ...
    models = {
        ...
        'GS752TP': 'Netgear',
        'GS752TPV2': 'Netgear',
        'Netgear M4100-50-POE': 'Netgear',
        'SF300-24PP-K9': '?',
        'WS-C2960-24PC-S': 'Cisco',
        'WS-C2960-48PST-S': 'Cisco',
        'WS-C2960G-24TC-L': 'Cisco',
        'WS-C2960L-48PS-LL': 'Cisco',
    }
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        ip = str(row[11])
        model = str(row[5])
        # validar IP e confirmar PRESI OK
        if validate_ip(ip) and str(row[12]).upper() == 'SIM':
            if models.get(f'{model}') == 'Cisco':
                equipment = {
    ...
    input(f'Equipments to work with: {len(device_list)}\nPress <ENTER> to continue...')
    ...
    try:
        with netmiko.ConnectHandler(
            **equipment,
            ...
            connection.exit_config_mode(pattern='-SW')
